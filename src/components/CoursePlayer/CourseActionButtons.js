...
export default function CourseActionButtons() {
  const { isHasAssignment, isHasQuiz, quizzesSubmitted, assignmentMark } =
    useQuizAndAssignment();

  const [modalIsOpen, setIsOpen] = useState(false);

  const navigate = useNavigate();

  const isShownQuizBtn = quizzesSubmitted || !isHasQuiz;
  ...
  return (
    <div className="flex gap-4">
      ...
      <button
        onClick={() => navigate(quizzesSubmitted ? "/" : "/quiz")}
